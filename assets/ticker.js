(function () {
  ...
  function buildCard(m, isLatest) {
    const el = document.createElement("article");
    el.className = "ticker-card" + (isLatest ? " ticker-card--latest" : "");
    const dateClass = "ticker-card__date" + (isLatest ? " ticker-card__date--latest" : "");
    const latestBadge = isLatest
      ? '<span class="ticker-card__latest-badge"><i class="fa fa-futbol-o" aria-hidden="true"></i><span>novo</span></span>'
      : "";
    el.innerHTML =
      '<div class="' +
      dateClass +
  ...
  function renderSets(matches) {
    ...
    function appendCycle() {
      for (let i = 0; i < matches.length; i++) {
        setA.appendChild(buildCard(matches[i], i === 0));
      }
    }
  ...
  function initialRightAlignedPosition() {
    const vw = viewport.clientWidth || 0;
    const cardWidth = cardStepPx();
    // Show latest at the right edge, with next-most-recent entering from right.
    return -segmentWidth + (vw - cardWidth);
  }
  ...
  async function load() {
    try {
      ...
      track.hidden = false;
      const orderedMatches = matches.slice().reverse();
      renderSets(orderedMatches);
      measure();
      position = initialRightAlignedPosition();
      wrapPosition();
      track.style.transform = "translateX(" + position + "px)";
